...
from src.loss_visualization import TrainingDashboard
...
def optimize(data_loaders, model, optimizer, loss, n_epochs, save_path, interactive_tracking=False):
    ...
    scheduler  = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer,factor = 0.33, patience = 2)
    ...
    for epoch in range(1, n_epochs + 1):
        ...
        if interactive_tracking:
            liveloss.update_training(train_loss)
            liveloss.update_validation(valid_loss)
...
def data_loaders():
    ...
    return get_data_loaders(batch_size=32, limit=200, valid_size=0.5, num_workers=4)
