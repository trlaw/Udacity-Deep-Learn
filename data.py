...
def get_data_loaders(
    batch_size: int = 32, valid_size: float = 0.2, num_workers: int = 4, limit: int = -1
):
    """
    ...
    data_transforms = {
        "train": transforms.Compose([
            ...
            # Do not believe this is technically transfer learning since just learned strategy to transform
            # our problem's input data.  Weights still from scratch.
...
def visualize_one_batch(data_loaders, max_n: int = 5):
    ...
    # Inverted dictionary using https://stackoverflow.com/a/483685
    class_names  = dict((c_idx,c_name) for (c_name,c_idx) in data_loaders["train"].dataset.class_to_idx.items())
